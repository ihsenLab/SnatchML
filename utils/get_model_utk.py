...
class SimpleModel(nn.Module):
    def __init__(self, in_channels, num_classes, expand=1.0, idx=14): # expland: []
        ...
        #fv = [144, 288, 432, 576, 576, 720, 864, 1008, 1152, 1152, 1296, 1440, 1584, 1728, 12544] # For UTKFace--reprogramming
        expands = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 1.0]
        ...
        self.fc_1 = nn.Linear(fv[expands.index(expand)], base_ch*2)
        self.relu = nn.ReLU()
...
def train_model(model, num_epochs, optimizer, criterion, train_loader, val_loader, test_loader, len_train, len_val, len_test, device):
    ...
    for epoch in range(num_epochs):
        ...
        for images, classes, _ in train_loader:
        ...
        for images, classes, _ in val_loader:
            images = images.to(device)
            classes = classes.to(device).long()
    ...
    with torch.no_grad():
        for images, classes, _ in test_loader:
            images = images.to(device)
            classes = classes.to(device).long()
